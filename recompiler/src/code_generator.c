...
static void emit_file_header(FILE *f) {
    ...
    /* g_hybrid_cycle_counter: cycle counter reported to clownmdemu-core via
     * bus-access callbacks for YM2612/Z80/VDP sync. Defined in runner/glue.c;
     * bumped per-instruction here using the same cycle-table value as
     * g_cycle_accumulator, so clownmdemu sees accurate inter-write timing
     * (not a 10-per-bus-access heuristic). */
    fprintf(f, "extern uint32_t g_hybrid_cycle_counter;\n");
    if (s_reverse_debug) {
